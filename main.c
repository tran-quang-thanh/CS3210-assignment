    ...
    if (rank == 0) {
        ...
        if (n) {
            ...
            while (i < num_files + 2) {
                ...
                char *temp_dir_name = malloc(100);
                strcpy(temp_dir_name, input_files_dir);
                strcat(temp_dir_name, filename);
                fp = fopen(temp_dir_name, "r");
                if (fp == NULL) {
        ...
        if (n) {
            ...
            while (i < num_files) {
                ...
                text = (char *) malloc(file_sizes[i] + 1); // temp buffer for the text read
